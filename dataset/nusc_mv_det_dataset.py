...
class NuscMVDetDataset(Dataset):
    def __init__(self,
        ...
        if self.return_height == True:
            self.height_sup_conf = height_sup_conf
    ...
    def sample_bda_augmentation(self):
        ...
        # rotate_bda = np.random.uniform(-5.4,5.4)
        rotate_bda = 0
        scale_bda = 1.0
        flip_dx = False
        flip_dy = np.random.uniform() < 0.5
        # flip_dy = False
        return rotate_bda, scale_bda, flip_dx, flip_dy
    
    def apply_bev_box_to_heatmap(self, heatmap, centerx, centery, width, height, k):
        half_width = (width / 2)
        half_height = (height / 2)

        x1 = int(max(centerx - half_width, 0))
        x2 = int(min(centerx + half_width, heatmap.shape[1]))
        y1 = int(max(centery - half_height, 0))
        y2 = int(min(centery + half_height, heatmap.shape[0]))
        
        heatmap[y1:y2, x1:x2] = k
        # masked_heatmap = heatmap
        return heatmap 
     
    def apply_bev_init_to_heatmap(self, heatmap, centerx, centery, width, height, k):
        half_width = (width / 2)
        half_height = (height / 2)
        ...
        heatmap[y1-k:y2+k, x1-k*3:x2+k*3] = 0.1
        #masked_heatmap = heatmap[y1:y2, x1:x2]
        return heatmap 

    def get_image(self, cam_infos, cams, gt_boxes):
        ...
        for cam in cams:
            ...
            for sweep_idx, cam_info in enumerate(cam_infos):
                ...
                extrin_mats.append(sweepego2sweepsensor)
        ...
        ret_list = [
            ...
            torch.stack(sweep_extrin_mats).permute(1, 0, 2, 3),
    ...
    def __getitem__(self, idx):
        ...
        for key_idx in self.key_idxes:
            ...
            cam_infos.append(info['ann_infos'])
        ...
        if self.is_train:
            if self.is_bda:
                ...
                rotate_bda, scale_bda, flip_dx, flip_dy = 0, 1.0, False, False
        else:
            rotate_bda, scale_bda, flip_dx, flip_dy = 0, 1.0, False, False
        ...
        bda_mat[:3, :3] = bda_rot
        image_data_list = self.get_image(cam_infos, cams, gt_boxes)
        ret_list = list()
        (
            sweep_imgs,
...
def collate_fn(data, is_return_depth=False, is_return_height=False):
    imgs_batch = list()
    sensor2ego_mats_batch = list()
    intrin_mats_batch = list()
    extrin_mats_batch = list()
    ida_mats_batch = list()
    ...
    mats_dict['extrin_mats'] = torch.stack(extrin_mats_batch)
