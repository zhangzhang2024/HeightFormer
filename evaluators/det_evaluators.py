...
class RoadSideEvaluator():
    ...
    def evaluate(
        ...
        results_path_dair = "outputs_dair" 
        results_path_rope = "outputs_rope" 
        # if 'dair' in self.data_root:
        if 'dair-v2x-i' in self.data_root:
            pred_label_path = result2kitti(result_files["img_bbox"], results_path_dair, self.data_root, self.gt_label_path, demo=False)
        else:
            pred_label_path = result2kitti_rope3d(result_files["img_bbox"], results_path_rope, self.data_root, self.gt_label_path, demo=False)
