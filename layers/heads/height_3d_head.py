...
__all__ = ['Height3DHead']

bev_decoder_conf = dict(
    type='Height3DNeck',
    in_channels=64*4,
    out_channels=128,
    num_layers=2,
...
class Height3DHead(CenterHead):
    ...
    def __init__(
        ...
        train_cfg=None,
        test_cfg=None,
        bev_decoder_conf=bev_decoder_conf,
        separate_head=dict(type='SeparateHead',
                           init_bias=-2.19,
                           final_kernel=3),
        is_train_height=False,
    ):
        super(Height3DHead, self).__init__(
        ...
        self.neck = build_neck(bev_decoder_conf)
    ...
    def forward(self, x):
        ...
        if self.is_train_height == True:
            x, pred_height= self.neck(x)
            ret_values = super().forward([x])
            return ret_values, pred_height
